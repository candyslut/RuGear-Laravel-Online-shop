<x-shop-layout>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start mb-16">
        ...
        <div class="flex flex-col h-full justify-between py-2 space-y-8">
            ...
            <div>
                ...
                @auth
                    @livewire('product-cart-counter', ['product' => $product])
                @else
    ...
    <style>
        /* ════════════════════════════════════════════════════════════
           СВЕТЛАЯ ТЕМА · акцент на контентных блоках (страница товара)
           Тёмные классы перекрашиваются в светлые (border-gray-800 →
           #e2e5ea), из-за чего белые карточки сливались с фоном. Для
           светлой темы усиливаем границы карточек, поднимаем блок
           описания мягкой тенью и делаем надпись на рыжей кнопке белой.
           Целимся в поверхности карточек (по bg-классу), чтобы не
           задеть оранжевые фокус-рамки полей. Скоуп — <main>.
           ──────────────────────────────────────────────────────────── */
        [data-theme="light"] main .bg-\[\#161920\],
        [data-theme="light"] main .bg-\[\#161920\]\/50 { border-color: #cdd4e0; }

        /* Блок описания без тени — поднимаем, как соседние карточки */
        [data-theme="light"] main .bg-\[\#161920\]\/50 {
            box-shadow: 0 1px 3px rgba(30,41,59,.06), 0 8px 20px -10px rgba(30,41,59,.15);
        }

        /* Рыжая кнопка «Добавить в корзину»: белая надпись вместо чёрной */
        [data-theme="light"] main button.bg-orange-500 { color: #fff; }
    </style>
</x-shop-layout>