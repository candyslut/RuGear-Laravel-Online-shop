            ...
            <div>
                @auth
                    ...
                    @if($itemInCart)
                        <div class="flex items-center justify-between bg-[#161920] border border-gray-800 rounded-xl p-2 shadow-lg">
                            <form action="{{ route('cart.remove', $product) }}" method="POST">
                                @csrf
                                <button class="w-12 h-12 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-800/50 rounded-lg transition-colors cursor-pointer">
                    ...
                    @else
                        <form action="{{ route('cart.add', $product) }}" method="POST">
                            @csrf
                            <button class="w-full py-4 bg-orange-500 hover:bg-orange-400 text-black font-extrabold uppercase text-xs tracking-widest rounded-xl transition-all shadow-md cursor-pointer">
                                Добавить в корзину
                            </button>
                        </form>
                    @endif
